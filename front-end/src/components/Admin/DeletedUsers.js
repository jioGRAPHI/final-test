...
class DeletedUsers extends Component {
  ...
  constructor(props) {
    ...
    fetch('http://localhost:3001/get-deleted-users')
    .then((response) => {return response.json() })
    .then((body) => {
      if(body.statusCode === 200){
        this.setState({ 
          deleted_users: body.deleted_users,
          search_result: body.deleted_users
        })
      }
    })
  }
  ...
  searchUser(){
    fetch('http://localhost:3001/find-deleted-user-by-username/'+ this.state.search_keyword)
    .then((response) => {return response.json() })
    .then((body) => {
      if(body.statusCode === 200){
        this.setState({ 
  ...
  clearResult(e){
    this.setState({
      search_result: this.state.deleted_users,
      clearResultButtonVisibility: false
    })
  }
